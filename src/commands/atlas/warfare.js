...
const { ANCESTRIES, BUILDINGS, STAT_MAPPING, STAT_KEYS } = require('../../data/constants');
...
function calcArmyPower(user, context) {
    ...
    let raw = 0;
    if (context === 'field')      raw = inf * 1.0 + cav * 1.5 + rng * 1.0 + sig * 0.0;
    else if (context === 'siege_atk') raw = inf * 1.0 + cav * 0.8 + rng * 1.0 + sig * 2.0;
    else if (context === 'siege_def') raw = inf * 1.0 + cav * 0.8 + rng * 1.2 + sig * 0.5;
...
async function handleBattleInitiate(interaction) {
    ...
    const foodCost = fieldFoodCost(atk);
    if ((atk.food_surplus || 0) < foodCost)
        return interaction.editReply({ content: `⚠️ Insufficient supplies. Need **${foodCost} 🥩** to march.` });
    ...
    const emb = new EmbedBuilder()
        ...
        .setDescription([
            ...
            `**Target:** <@${def.id}>`,
            '',
            `⚔️ Inf: ${atk.mil_infantry || 0} | 🐎 Cav: ${atk.mil_cavalry || 0} | 🏹 Rng: ${atk.mil_ranged || 0} | 🗡️ Mercs: ${atk.mercs_temp || 0}`,
            `Morale: ${calcMorale(atk)} | Food spent: ${foodCost} 🥩`,
        ].join('\n'));

    const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`warapprove_battle_${atk.id}_${def.id}`).setLabel('✅ Approve Battle').setStyle(ButtonStyle.Success),
    ...
    return interaction.editReply({ content: `⚔️ Battle request submitted. **${foodCost} 🥩** supply spent. Awaiting GM approval.` });
}

// ─── BATTLE APPROVE (GM) ───────────────────────────────────────────────────────

async function handleBattleApprove(interaction, atkId, defId) {
    const db = interaction.client.db;
    ...
    // Deduct defender food
    const defFood = fieldFoodCost(def);
    ...
    const counter = compCounterBonus(atk, def);

    const atkPower = calcArmyPower(atk, 'field') + atkOff * 5 + menMod * 2 + polyB + counter + atkRoll + agBonusAtk;
    const defPower = calcArmyPower(def, 'field') * 1.2 + (await calcOffenseScore(db, defId)) * 5 + defRoll + agBonusDef;
    ...
    const lInf = Math.max(0, Math.floor((loser.mil_infantry || 0) * lossFactor));
    const lCav = Math.max(0, Math.floor((loser.mil_cavalry  || 0) * lossFactor));
    const lRng = Math.max(0, Math.floor((loser.mil_ranged   || 0) * lossFactor));

    await db.run(
        'UPDATE users SET mil_infantry=?, mil_cavalry=?, mil_ranged=?, rate_prest=MAX(-10,rate_prest-2), rate_stab=MAX(-10,rate_stab-1), mil_maintenance_cost=? WHERE id=?',
        lInf, lCav, lRng, calcMaintenance({ ...loser, mil_infantry: lInf, mil_cavalry: lCav, mil_ranged: lRng }), loserId
    );
    await db.run(
        'UPDATE users SET rate_prest=MIN(10,rate_prest+1) WHERE id=?', winnerId
    );

    // Log gm_events
    await db.run('INSERT INTO gm_events (user_id, gm_id, event_type, severity, effect_snapshot, created_at) VALUES (?,?,?,?,?,?)',
        winnerId, interaction.user.id, 'field_battle', 1, JSON.stringify({ atk: atkId, def: defId, winner: winnerId }), Date.now());

    // Disable buttons on GM message
    await interaction.update({ components: [], content: `✅ Battle resolved. **<@${winnerId}>** wins!` });
    ...
    for (const uid of [atkId, defId]) {
        ...
        const emb = new EmbedBuilder()
            .setTitle(isWinner ? '🏆 VICTORY' : '💀 DEFEAT')
            .setColor(isWinner ? 0x00FF88 : 0xFF0000)
            .setDescription([
                `Battle against <@${isWinner ? loserId : winnerId}>`,
                `Atk roll: ${atkRoll} → Power: ${atkPower}`,
                `Def roll: ${defRoll} → Power: ${defPower}`,
                '',
                isWinner ? '+1 Prestige' : `-2 Prestige, -1 Stability\n⚔️ Inf: ${lInf} | 🐎 Cav: ${lCav} | 🏹 Rng: ${lRng}`,
            ].join('\n'));
        let sent = false;
    ...
    // 20% chance substat pick for winner
    if (Math.random() < 0.20) {
...
async function handleSiegeConfirm(interaction, atkId, defId, townId) {
    ...
    let atkPower = calcArmyPower(atk, 'siege_atk') + atkOff * 5 + menMod * 2 + polyB + atkRoll + agAtk;
    let defPower = calcArmyPower(def, 'siege_def') * 1.2 + defScore * 8 + terrainB + styxBonus + defRoll + agDef;
    ...
    await db.run('INSERT INTO gm_events (user_id, gm_id, event_type, severity, effect_snapshot, created_at) VALUES (?,?,?,?,?,?)',
        winner.id, interaction.user.id, 'siege', 1, JSON.stringify({ atk: atkId, def: defId, winner: winner.id }), Date.now());
...
async function handleButton(interaction, action, args) {
    if (action === 'warapprove' && args[0] === 'battle')
        return handleBattleApprove(interaction, args[1], args[2]);
...
module.exports = { handleBattleInitiate, handleSiegeInitiate, handleButton, handleRebellionEvent };
