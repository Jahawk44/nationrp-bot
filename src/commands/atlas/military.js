...
const { getMod, getPlayerRank, calcMaintenance, calcMorale, ephemeralReply } = require('../../utils/helpers');
...
async function handleSelect(interaction, action, args) {
    ...
    if (action === 'mil') {
        const sub = args[0];
        const uid = args[1];
        if (interaction.user.id !== uid) return ephemeralReply(interaction, '⚠️ Only the commander may use this.');
        ...
        if (sub === 'recruittype') {
            ...
            const cost = unitType.toUpperCase() === 'MERCENARY' ? 150 : def.cost_balance;
        ...
        if (sub === 'battleplayer') {
            ...
            if (!def) return ephemeralReply(interaction, '⚠️ Target not found.');
            const towns = await db.get('SELECT COUNT(*) as cnt FROM towns WHERE user_id=?', uid);
            const hasTown = (towns?.cnt || 0) > 0;
            if (!atk.nation && !hasTown) return ephemeralReply(interaction, '⚠️ You need at least a settlement to declare battle.');
            await interaction.deferUpdate();
        ...
        if (sub === 'formation') {
            ...
            if (!formation) return ephemeralReply(interaction, '⚠️ Unknown formation.');
            ...
            if (formation.reqUnit) {
                ...
                if (reqCount === 0)
                    return ephemeralReply(interaction, `⚠️ **${formation.name}** requires **${formation.reqName}** — you don't own any. Recruit them first via \`/atlas military\` → Recruit.`);
            }
...
async function handleMilSiege(interaction, uid, targetId, townName, db, warfare) {
    ...
    for (const f of fields) {
        ...
        if (val < 0) return ephemeralReply(interaction, '⚠️ Values must be positive.');
        if (val > (atk[colMap[f]]||0)) return ephemeralReply(interaction, `⚠️ Cannot commit more ${f} than you own.`);
        counts[f] = val;
    }
    ...
    return ephemeralReply(interaction, `🏰 Siege submitted. ${foodCost} 🥩 spent. Awaiting GM.`);
}
...
async function handleFormationSubmit(interaction, uid, targetId, formationKey, mode, townName, db) {
    ...
    return ephemeralReply(interaction, `⚔️ ${mode === 'siege' ? 'Siege' : 'Battle'} submitted. ${foodCost} 🥩 spent. Awaiting GM. Formation: **${formation.name}**.${counterNote}`);
}
